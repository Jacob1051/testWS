...
public class Signalement {
    ...
    public Signalement[] getHistoryOfReport(String id)
    {
        ...
        try{
            stm=con.getCon().createStatement();
            rs=stm.executeQuery("SELECT * FROM Signalement s WHERE s.idUtilisateur = '"+id+"'");
            while(rs.next())
            {
    ...
    public String[] getStatutSignalement(String id)
    {
        ...
        try{
            stm=con.getCon().createStatement();
            String query = "SELECT s.*,(CASE WHEN s.id IN (SELECT idSignalement FROM Valide) THEN 'valide' ELSE 'invalide' END) as Statut FROM Signalement s WHERE s.idUtilisateur='"+id+"';";
            ...
            Object[] array = v.toArray();
            retour = Arrays.copyOf(array, array.length, String[].class);
